...
import { updateRecurringPayment } from "./index";
...
describe("updateRecurringPayment procedure", () => {
  it("returns the updated recurring payment when the row exists", async () => {
    mock.db = buildSupabaseMock({
      user,
      tables: { recurring_payments: { data: recurringRow } },
    });

    const result = await updateRecurringPayment(validInput, buildContext());
