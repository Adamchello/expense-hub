...
interface BillImportBodyProps {
  /** When false the import wizard resets (mirrors the old close-resets behavior). */
  active: boolean;
  onDone: () => void;
}

export function BillImportBody({ active, onDone }: BillImportBodyProps) {
  const { data: existingBills } = useBills({ enabled: active });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <BillImportBody active={open} onDone={() => onOpenChange(false)} />
      </DialogContent>
    </Dialog>
  );
}
