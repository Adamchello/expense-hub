...
beforeEach(() => {
  ...
  queryClient.setQueryData(["expenses"], [EXISTING_EXPENSE]);
  server.use(
    http.get("/api/expenses/list", () =>
      HttpResponse.json({ data: [EXISTING_EXPENSE] }),
    ),
    http.post("/api/expenses/import", () => HttpResponse.json({ imported: 2 })),
  );
});
...
describe("Duplicate Detection", () => {
  it("flags row matching existing expense on provider, amount, and date", async () => {
    const csv = [
